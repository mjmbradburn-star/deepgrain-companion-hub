...
Deno.test('score-responses: malformed token returns a safe auth state', async () => {
  const res = await fetch(`${SUPABASE_URL}/functions/v1/score-responses`, {
    method: 'POST',
    headers: {
    ...
    body: JSON.stringify({ respondent_id: '00000000-0000-0000-0000-000000000000' }),
  })

  await assertSafeUnauthorized(res, 'Malformed score-responses token')
})

Deno.test('score-responses: expired-looking token returns a safe auth state', async () => {
  const res = await fetch(`${SUPABASE_URL}/functions/v1/score-responses`, {
    method: 'POST',
    headers: {
      apikey: SUPABASE_ANON_KEY,
  ...
  await assertSafeUnauthorized(res, 'Expired score-responses token')
})